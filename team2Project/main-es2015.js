...
const routingComponents = [_allocate_allocate_component__WEBPACK_IMPORTED_MODULE_3__["AllocateComponent"],
...
let ApplicationRecordsComponent = class ApplicationRecordsComponent {
...
class FeedbackData {
    constructor(candidateName, email, phoneNo, programmingLanguage, status, additionalComments) {
...
let FeedbackFormComponent = class FeedbackFormComponent {
    ...
    goPrevious() {
        let previousId = this.applicationRecordId - 1;
        this.router.navigate(['/feedback/empId', previousId]);
    }
    goNext() {
        let nextId = this.applicationRecordId + 1;
        this.router.navigate(['/feedback/empId', nextId]);
    }
    goToApplicationRecords() {
        let selectedId = this.applicationRecordId ? this.applicationRecordId : null;
